...
function migrateLegacyBrowserStorage(): void {
  ...
  try {
    const keys = Array.from({ length: localStorage.length }, (_, index) => localStorage.key(index))
      .filter((key): key is string => Boolean(key?.startsWith('portfolio-ai:')));
    for (const key of keys) {
      const value = localStorage.getItem(key);
      if (value !== null && sessionStorage.getItem(key) === null) sessionStorage.setItem(key, value);
      localStorage.removeItem(key);
    }
  } catch {
    // Server-side encrypted storage remains authoritative.
  }
}
...
function persistSettings(settings: StoredSettings, options: SavePositionsOptions = {}) {
  void fetch('/api/settings', {
    ...
    if (settings.positions !== undefined && sessionStorage.getItem(POSITIONS_KEY) === JSON.stringify(settings.positions)) {
      sessionStorage.removeItem(POSITIONS_DIRTY_KEY);
    }
  }).catch(() => {
...
export function savePositions(positions: UserPosition[], options: SavePositionsOptions = {}) {
  ...
  try {
    ...
    sessionStorage.setItem(POSITIONS_DIRTY_KEY, '1');
  } catch {
...
function saveBrowserSnapshot(positions: UserPosition[]) {
  try {
    const raw = sessionStorage.getItem(POSITION_BACKUPS_KEY);
    ...
    sessionStorage.setItem(POSITION_BACKUPS_KEY, JSON.stringify(snapshots.slice(-100)));
  } catch {
...
export function saveRealEstate(properties: unknown[]): void {
  try { sessionStorage.setItem(RE_KEY, JSON.stringify(properties)); } catch {}
  void fetch('/api/settings', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export function saveOtherAssets(assets: unknown[]): void {
  try { sessionStorage.setItem(OTHER_KEY, JSON.stringify(assets)); } catch {}
  void fetch('/api/settings', {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export async function hydrateSettings(): Promise<StoredSettings> {
  ...
  const localPositionsAreNewer = sessionStorage.getItem(POSITIONS_DIRTY_KEY) === '1';
  const localProperties = (() => { try { return JSON.parse(sessionStorage.getItem(RE_KEY) ?? 'null') as unknown[] | null; } catch { return null; } })();
  const localOtherAssets = (() => { try { return JSON.parse(sessionStorage.getItem(OTHER_KEY) ?? 'null') as unknown[] | null; } catch { return null; } })();
  try {
    ...
    if (positions) sessionStorage.setItem(POSITIONS_KEY, JSON.stringify(positions));
    ...
    if (localProperties?.length) {
      ...
      sessionStorage.setItem(RE_KEY, JSON.stringify(server.properties));
    }
    ...
    if (localOtherAssets?.length) {
      ...
      sessionStorage.setItem(OTHER_KEY, JSON.stringify(server.otherAssets));
    }
...
export function savePortfolioCache(data: unknown): void {
  if (typeof window === 'undefined') return;
  try {
    sessionStorage.setItem(PORTFOLIO_CACHE_KEY, JSON.stringify({ d: data, t: Date.now() }));
  } catch {}
}
...
export function loadAllTheses(): ThesisEntry[] {
  if (typeof window === 'undefined') return [];
  try {
    const raw = sessionStorage.getItem(THESIS_KEY);
...
export function saveThesis(entry: ThesisEntry) {
  ...
  sessionStorage.setItem(THESIS_KEY, JSON.stringify(all));
}
...
export function hasOnboarded(): boolean {
  ...
  const pos = sessionStorage.getItem(POSITIONS_KEY);
...
export function saveMetricSnapshot(snapshot: Omit<MetricSnapshot, 'date'>): void {
  ...
  try {
    ...
    sessionStorage.setItem(METRIC_HISTORY_KEY, JSON.stringify(history.slice(-30)));
  } catch {}
}
